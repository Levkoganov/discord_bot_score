    ...
    try {
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === p1.username) {
          ...
          // Player1 win
          if (p1_score === 10) {
            cardEmbed.fields[0].value = `*${p1.username}(**${p1_score}**)*`; // Winner field
            cardEmbed.fields[2].value = `*~~${p2.username}(**${p2_score}**)~~*`; // Loser field
          ...
          cardEmbed.fields[0].value = `${p1.username}(***${p1_score}***)`; // Edit embed field
        ...
        if (i.customId === p2.username) {
          ...
          // Player2 win
          if (p2_score === 10) {
            cardEmbed.fields[2].value = `*${p2.username}(**${p2_score}**)*`; // Winner field
            cardEmbed.fields[0].value = `*~~${p1.username}(**${p1_score}**)~~*`; // Loser field
          ...
          // Update player1 embed
          cardEmbed.fields[2].value = `${p2.username}(***${p2_score}***)`; // Edit embed field
          await i.update({ embeds: [cardEmbed] });
        }
      });
